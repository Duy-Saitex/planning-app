...
function makeBlock(it, rowH, blkH, stage) {
  ...
  const b = h('div', {
    ...
    oncontextmenu: e => { e.preventDefault(); e.stopPropagation(); blockMenu(o, e); },
    onkeydown: e => {
...
const GLIDE = { duration: 320, easing: 'cubic-bezier(.2,.7,.3,1)', fill: 'backwards' };

// Where every lane and every bar sits right now.
function boardSnapshot() {
  const lanes = new Map(), blocks = new Map();
  for (const l of document.querySelectorAll('.lane')) if (l._lane) lanes.set(l._lane.name, l.getBoundingClientRect());
  for (const b of document.querySelectorAll('.blk')) if (b._o) blocks.set(b._o.r, b.getBoundingClientRect());
  return { lanes, blocks };
}
// A copy of the bar that is leaving, pinned where it was, shrinking and fading
// after the real one has already gone.
...
// Lanes that shifted glide to their new place; bars that changed row within a
// lane glide by the remainder. Nested FLIP, driven by the Web Animations API so
// it does not depend on style-recalc ordering or a frame callback.
function flipBoard(before) {
  // Read every position first. Starting an animation mid-measurement would
  // shift the rects of everything read after it.
  const laneMoves = [], laneDy = new Map();
  for (const l of document.querySelectorAll('.lane')) {
    if (!l._lane) continue;
    const was = before.lanes.get(l._lane.name); if (!was) continue;
    const dy = was.top - l.getBoundingClientRect().top;
    laneDy.set(l._lane.name, dy);
    if (dy && Math.abs(dy) < 900) laneMoves.push([l, dy]);
  }
  const barMoves = [];
  for (const b of document.querySelectorAll('.blk')) {
    if (!b._o) continue;
    const was = before.blocks.get(b._o.r); if (!was) continue;
    const now = b.getBoundingClientRect();
    const lane = b.closest('.lane');
    const ldy = lane && lane._lane ? (laneDy.get(lane._lane.name) || 0) : 0;
    const dx = Math.round(was.left - now.left), dy = Math.round((was.top - now.top) - ldy);
    if ((!dx && !dy) || Math.abs(dx) > 900 || Math.abs(dy) > 900) continue;
    barMoves.push([b, dx, dy]);
  }
  // Then write.
  for (const [l, dy] of laneMoves) l.animate([{ transform: `translateY(${dy}px)` }, { transform: 'none' }], GLIDE);
  barMoves.forEach(([b, dx, dy], i) =>
    b.animate([{ transform: `translate(${dx}px,${dy}px)` }, { transform: 'none' }], { ...GLIDE, delay: Math.min(i * 14, 110) }));
}
function markEntering(row) {
  if (reducedMotion()) return;
  const el = [...document.querySelectorAll('.blk')].find(b => b._o && b._o.r === row);
  if (el) el.animate([{ opacity: 0, transform: 'scale(.88)' }, { opacity: 1, transform: 'none' }],
...
// Right-click on a bar: the two ways to get it out of the way, plus open.
function blockMenu(o, e) {
  const m = S.model, F = m.F;
  const lane = laneFieldForBoard(), cur = str(o.v[lane.col]).trim();
